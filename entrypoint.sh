...
get_token() {
  echo "Generate Gateway Token for API Version:v1.3"  
	TOKEN=$($CURL -k -X POST ''${QUALYS_API_SERVER}'/auth' -d 'username='${USERNAME}'&password='${PASSWORD}'&token=true&permissions=true' -H 'Content-Type:application/x-www-form-urlencoded' -s)
}
...
SHA=$(docker inspect $IMAGE_ID | grep Id | sed 's/"//g' | cut -d: -f3 | sed 's/,//g')
